...
class TechnologyController extends BasicController {
    ...
    public function tabPageAction(){
        try{
            ...
            $data=$this->m_technology->getListByPage($limit,$page,$sort,$_POST['searchPhrase']);
    ...
    public function editAction(){
        $data=$this->m_technology->getfiledById($this->getRequest()->get('id'));
    ...
    public function updateByIdAction(){
        try{
	    	...
	    	//print_r($_POST);exit;
            $res=$this->m_technology->UpdateByID($post,$this->getRequest()->getpost('id'));
            if($res){
                $resouce['code']=0;
                $resouce['msg']='修改成功';
            }else{
    ...
    public function addhtmlAction(){
        $data=$this->m_technology->getfiledall();
    ...
    public function addinsertAction(){
        try{
            $result = $this->m_technology->Deleteall();
            $post['title']=filterStr($this->getRequest()->getpost('title'));
            $post['img']=serialize($this->getRequest()->getpost('urllogo'));
	    	$post['c_time']=date('Y-m-d H:i:s');
	    	$post['info']=$this->getRequest()->getpost('info');
	    	
            $res=$this->m_technology->Insert($post);
            if($res){
                $resouce['code']=0;
                $resouce['msg']='添加成功';
            }else{
    ...
    public function deleteAction(){
        try{
            ...
            if(is_numeric($id)){
                $result = $this->m_technology->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }else{
                $result = $this->m_technology->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }
