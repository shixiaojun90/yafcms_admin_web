...
class ContactController extends BasicController {
    ...
    public function tabPageAction(){
        try{
            ...
            $data=$this->m_contact->getListByPage($limit,$page,$sort,$_POST['searchPhrase']);
    ...
    public function editAction(){
        $data=$this->m_contact->getfiledById($this->getRequest()->get('id'));
    ...
    public function updateByIdAction(){
        try{
	    	...
	    	//print_r($_POST);exit;
            $res=$this->m_contact->UpdateByID($post,$this->getRequest()->getpost('id'));
            if($res){
                $resouce['code']=0;
                $resouce['msg']='修改成功';
            }else{
    ...
    public function addinsertAction(){
        try{
            $result = $this->m_contact->Deleteall();
            
            $post['name']=filterStr($this->getRequest()->getpost('name'));
            $post['address']=filterStr($this->getRequest()->getpost('address'));
            $post['tel']=filterStr($this->getRequest()->getpost('tel'));
            $post['email']=filterStr($this->getRequest()->getpost('email'));
	    	$post['c_time']=date('Y-m-d H:i:s');
	    	
            $res=$this->m_contact->Insert($post);
            if($res){
                $resouce['code']=0;
                $resouce['msg']='添加成功';
            }else{
    ...
    public function deleteAction(){
        try{
            ...
            if(is_numeric($id)){
                $result = $this->m_contact->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }else{
                $result = $this->m_contact->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }
