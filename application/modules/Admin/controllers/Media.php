...
class MediaController extends BasicController {
    ...
    public function tabPageAction(){
        try{
            ...
            $data=$this->m_media->getListByPage($limit,$page,$sort,$_POST['searchPhrase']);
    ...
    public function editAction(){
        $data=$this->m_media->getfiledById($this->getRequest()->get('id'));
    ...
    public function updateByIdAction(){
        try{
	    	...
	    	//print_r($_POST);exit;
            $res=$this->m_media->UpdateByID($post,$this->getRequest()->getpost('id'));
            if($res){
                $resouce['code']=0;
                $resouce['msg']='修改成功';
            }else{
    ...
    public function deleteAction(){
        try{
            ...
            if(is_numeric($id)){
                $result = $this->m_media->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }else{
                $result = $this->m_media->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }
