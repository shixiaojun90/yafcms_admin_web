...
class AuthController extends BasicController {
    ...
    public function tabPageAction(){
        try{
            ...
            $data=$this->m_auth->getListByPage($limit,$page,$sort,$_POST['searchPhrase']);
    ...
    public function editAction(){
        $data=$this->m_auth->getfiledById($this->getRequest()->get('id'));
    ...
    public function updateByIdAction(){
        try{
	    	...
	    	//print_r($_POST);exit;
            $res=$this->m_auth->UpdateByID($post,$this->getRequest()->getpost('id'));
            if($res){
                $resouce['code']=0;
                $resouce['msg']='修改成功';
            }else{
    ...
    public function addinsertAction(){
        try{
            //print_r($_POST);exit;
            $result = $this->m_auth->Deleteall();
            $post['title']=filterStr($this->getRequest()->getpost('title'));
            $post['img']=serialize($this->getRequest()->getpost('urllogo'));
	    	$post['c_time']=date('Y-m-d H:i:s');
	    	$post['info']=$this->getRequest()->getpost('info');
	    	
            $res=$this->m_auth->Insert($post);
            if($res){
                $resouce['code']=0;
                $resouce['msg']='添加成功';
            }else{
    ...
    public function deleteAction(){
        try{
            ...
            if(is_numeric($id)){
                $result = $this->m_auth->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }else{
                $result = $this->m_auth->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }
