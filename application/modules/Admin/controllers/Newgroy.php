 ...
 * Time: 11:29
 */
class NewgroyController extends BasicController {
    ...
    public function tabPageAction(){
        try{
            ...
            $data=$this->m_newgroy->getListByPage($limit,$page,$sort,$_POST['searchPhrase']);
    ...
    public function editAction(){
        $data=$this->m_newgroy->getfiledById($this->getRequest()->get('id'));
    ...
    public function updateByIdAction(){
        try{
	    	...
	    	//print_r($_POST);exit;
            $res=$this->m_newgroy->UpdateByID($post,$this->getRequest()->getpost('id'));
            if($res){
                $resouce['code']=0;
                $resouce['msg']='修改成功';
            }else{
    ...
    public function addinsertAction(){
        try{
            ...
            $res=$this->m_newgroy->Insert($post);
            if($res){
                $resouce['code']=0;
                $resouce['msg']='添加成功';
            }else{
    ...
    public function deleteAction(){
        try{
            ...
            if(is_numeric($id)){
                $result = $this->m_newgroy->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }else{
                $result = $this->m_newgroy->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }
