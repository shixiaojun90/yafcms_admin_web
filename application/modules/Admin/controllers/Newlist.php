...
class NewlistController extends BasicController {
    ...
    public function tabPageAction(){
        try{
            ...
            $data=$this->m_newlist->getListByPage($limit,$page,$sort,$_POST['searchPhrase']);
            
            foreach ($data['data'] as $itemk =>$itemv){
                $list[$itemk]=quotes($itemv);
                $list[$itemk]['cid']=$this->m_newgroy->getfiledById($itemv['cid'])['title'];
            }
    ...
    public function editAction(){
        $data=$this->m_newlist->getfiledById($this->getRequest()->get('id'));
        $data['categrouy']=$this->m_newgroy->getfiledall();
        //echo "<pre/>";
        //print_r($data);exit;
    ...
    public function updateByIdAction(){
        try{
            $post['cid']=$this->getRequest()->getpost('cid');
	    	$post['title']=filterStr($this->getRequest()->getpost('title'));
            $post['img']=$this->getRequest()->getpost('urllogo');
	    	$post['c_time']=date('Y-m-d H:i:s');
	    	$post['desc']=$this->getRequest()->getpost('desc');
	    	$post['info']=$this->getRequest()->getpost('info');
	    	$post['home']=$this->getRequest()->getpost('home');
	    	
	    	//print_r($_POST);exit;
            $res=$this->m_newlist->UpdateByID($post,$this->getRequest()->getpost('id'));
            if($res){
                $resouce['code']=0;
                $resouce['msg']='修改成功';
            }else{
    ...
    public function addhtmlAction(){
        $data=$this->m_newgroy->getfiledall();
    ...
    public function addinsertAction(){
        try{
            ...
            $res=$this->m_newlist->Insert($post);
            if($res){
                $resouce['code']=0;
                $resouce['msg']='添加成功';
            }else{
    ...
    public function deleteAction(){
        try{
            ...
            if(is_numeric($id)){
                $result = $this->m_newlist->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }else{
                $result = $this->m_newlist->Where("id in ({$this->getRequest()->getpost('id')})")->Delete();
            }
