 ...
 * Time: 上午10:42
 */
class CloudwordController extends BasicController {
    ...
    /*********创建文件***********/
    public function openwordAction(){
       try{
            ...
            if(!empty($pid) || $pid > 0){
                ...
                if(!empty($dirinfo['path'])){
                    $dirpath=date('YmdHis');
                    $path=$dirinfo['path'].'/'.$dirpath.".{$extend}";
                }
                //print_r($path);exit;
            }else{
                $dirpath=date('YmdHis');
                $path="cloud/{$uid}/{$dirpath}".".{$extend}";
            }
