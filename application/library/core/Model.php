...
//    }
     ...
     * @return array
     */
	final public  function tabPage($rowCount,$current,$sort='id desc',$search=null,$where=null){
		...
		if(is_array($sort)){
			if($search){
				$tsql='(select '.$this->primary.' from (select '.$this->primary.' from `'. $this->table. '` where '.($where!=null?' and '.$where:' ').$search.' order by `'.array_keys($sort)[0].'` '.$sort[array_keys($sort)[0]].' limit '.$startCount.','.$rowCount.')as t)';
				if(isset($this->options['conditions'])){
			...
			if($search){
				$tsql='(select '.$this->primary.' from (select '.$this->primary.' from `'. $this->table. '` where '.$str.($where!=null?' and '.$where:' ').$search.' order by '.$sort.' limit '.$startCount.','.$rowCount.')as t)';
				if(isset($this->options['conditions'])){
					$tsql.= $this->options['conditions'];
				}
				$tsql.=' order by  '.$sort;
        ...
        $this->selectOne = FALSE;
        if($search) {
            $countSql = 'SELECT count(*) as `total` FROM `' . $this->table . '` where '.($where!=null?' and '.$where:' ').$search;
        }else{
